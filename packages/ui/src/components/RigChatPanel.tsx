    ...
    <div data-testid="rig-chat-panel" className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {isLoading ? (
          ...
          messages.map((msg) => (
            <div key={msg.id} data-testid={`chat-msg-${msg.id}`} className="font-mono text-xs">
              {msg.kind === "topic" ? (
                <div className="text-stone-400 text-center text-[10px]">--- topic: {msg.topic} ---</div>
              ) : (
                <div>
                  <span data-testid={`chat-sender-${msg.id}`} className="font-bold text-stone-600">[{msg.sender}]</span>{" "}
                  <span>{msg.body}</span>
                </div>
              )}
            </div>
      ...
      <form data-testid="chat-send-form" onSubmit={handleSubmit} className="border-t border-stone-200 p-2 flex gap-2">
        <input
          ...
          className="flex-1 px-2 py-1 border border-stone-300 font-mono text-xs"
          disabled={isSending}
        />
        <button
          ...
          className="px-3 py-1 border border-stone-300 font-mono text-[10px] uppercase hover:bg-stone-200 disabled:opacity-50"
        >
          Send
        </button>
      </form>
